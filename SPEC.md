...
1. **消息定界：注入分隔标记 + 标记唯一化。** 每条 ExpectResult 命令写入 stdin 时，自动在其后
   ...
   MarkerFramer 缓冲设 1MB 上限：无换行/无标记残余超限即冲刷为帧并清空（防内存 DoS，
   代价是跨分片的部分标记可能被牺牲）。
   - `marker`（默认 `"\x1dDONE\x1d"`）与 `probeCommand`（模板，默认 Tcl 形式
     `puts "%1"`，`%1` 为完整期望标记；probe 不含 `%1` 时告警，因为 ExpectResult
     命令会全部挂超时）均可配置，以适配非 Tcl 程序。
...
5. **失败处理：错误回调 + 清空 + 有上限的自动重启。**
   ...
   - 单条命令超时：该命令以 `Timeout` 回调并出队，**不杀进程**，且结果带已收到的
     部分输出（与 ProcessDied 一致：CollectAll 聚合 join，否则最后一条）；
...
- `printhang <text>`：打印一行后永久挂起（用于超时保留部分输出测试）
- 其他输入：原样回显一行
...
- **批量执行辅助（BatchRunner）**：企业批量场景（步骤声明 + 依赖链 + 失败策略）目前以
  examples/batch_demo.cpp 模板形式提供，不收入库。理由：模式本身仅约 40 行直白代码，
  而真实使用者的中止/重试/并行/进度策略各不相同，正式 API 要么太死板要么长成参数地狱。
  **收入库的触发条件（rule of three）：当该模式在真实代码中出现第三次且形态趋同时**，
  再将公共部分提炼为辅助类；前两次使用中的差异正是抽象边界的依据。
